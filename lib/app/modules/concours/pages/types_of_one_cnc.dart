...
class TypeOfOneCnc extends StatefulWidget {
  String type;
  int ecole_id;
  TypeOfOneCnc({Key? key, required this.type, required this.ecole_id})
      : super(key: key);
      ...
      body: Column(
        children: [
          InkWell(
            ...
            child: FutureBuilder(
              future: controller.selectvilleecole(
                  "${widget.ecole_id}", widget.type),
                    ...
                    itemBuilder: (context, index) {
                      return InkWell(
                        onTap: () => Get.to(() => ConcoureContent(
                            niveaux: widget.type,
                            ville_id: "${snapshot.data[index]['id_ville']}",
                            ecole_id: widget.ecole_id)),
                        child: TypeConcoure(
                          url_img:
                              "$linkserverimages/ecolevilleLogo/${snapshot.data[index]['logo']}",
                          titel:
                              "${snapshot.data[index]['ville']}".toUpperCase(),
                          type: 2,
                        ),
                      );
                    },
